...
export class EventAuthorInviteEventGuestsHandler
  implements ICommandHandler<EventAuthorInviteEventGuestsCommand>
{
  constructor(
  ...
  async execute(command: EventAuthorInviteEventGuestsCommand) {
    try {
      ...
      await Promise.all(payload.guestIds.map(async(guestId) => {
        try {
          this.logger.log('[EVENT-AUTHOR-INVITE-EVENT-GUEST-HANDLER-PROCESSING]');
          ...
          if (payload.followupInvitations && payload.followupInvitations.length > 0) {
            await Promise.all(payload.followupInvitations.map(async(followupInvitation) => {
              try {
                this.logger.log('[EVENT-AUTHOR-FOLLOWUP-INVITATION-HANDLER-PROCESSING]');
                ...
                this.logger.log('[EVENT-AUTHOR-FOLLOWUP-INVITATION-HANDLER-SUCCESS]');
              } catch(error) {
                this.logger.log(`[EVENT-AUTHOR-FOLLOWUP-INVITATION-HANDLER-ERROR] :: ${error}`);
              }
            }));
          }
          
          this.logger.log('[EVENT-AUTHOR-INVITE-EVENT-GUEST-HANDLER-SUCCESS]');
        } catch(error) {
          this.logger.log(`[EVENT-AUTHOR-INVITE-EVENT-GUEST-HANDLER-ERROR] :: ${error}`);
      ...
      this.logger.log(`[EVENT-AUTHOR-INVITE-EVENT-GUESTS-HANDLER-ERROR] :: ${error}`);
