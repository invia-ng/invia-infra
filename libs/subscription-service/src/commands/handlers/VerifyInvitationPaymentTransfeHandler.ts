...
@CommandHandler(VerifyInvitationPaymentTransferCommand)
export class VerifyInvitationPaymentTransferHandler implements ICommandHandler<
  VerifyInvitationPaymentTransferCommand,
...
> {
  ...
  async execute(command: VerifyInvitationPaymentTransferCommand) {
    try {
      const { paymentReference, secureUser } = command;

      this.logger.log(
        `[VERIFY-INVITATION-PAYMENT-TRANSFER-COMMAND-HANDLER-PROCESSING]: ${JSON.stringify(command)}`,
      );
      ...
      this.logger.log(`[VERIFY-INVITATION-PAYMENT-TRANSFER-COMMAND-HANDLER-SUCCESS]`);
      ...
      this.logger.log(
        `[VERIFY-INVITATION-PAYMENT-TRANSFER-COMMAND-HANDLER-ERROR]: ${error}`,
      );
